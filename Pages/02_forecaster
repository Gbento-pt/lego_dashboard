...
with st.container():
    ...
    rmse_arima = np.sqrt(mean_squared_error(df_test.nbr_sets, preds_arima))
    st.text(f"On the test set, for RMSE is {rmse_arima}")
    ...
    webhook_url = st.text('Insert the Discord Webhook URL here!')
...
if st.button('Send Forecasts to Discord'):
    # Code to connect to the Webhook and send the forecasts
